...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  const handleUpdateProfile = () => {
    if (!userData) return;
    // FIX: pass a plain string instead of an object
  ...
  if (loading) return null; // still hydrating session
        ...
        {user ? (
          <>
            ...
            <div className="flex flex-col items-center pt-20 pb-6 border-b border-gray-100">
              <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-gray-100 to-white
                              mb-3 overflow-hidden shadow-md border-2 border-gray-200" />
              <h2 className="text-xl font-semibold mb-1">
                {`${userData?.first_name} ${userData?.last_name}` || 'User'}
              </h2>
              <p className="text-sm mb-4">
                tokens: {userData?.available_tokens ?? 0}
              </p>

              <button
                onClick={handleUpdateProfile}
                className="w-48 bg-white rounded-full py-2 px-4 text-sm
                           border border-gray-200 hover:border-gray-300
                           hover:bg-gray-50 transition-shadow shadow-sm
                           hover:shadow-md"
              >
                View Profile
              </button>
            ...
            <div className="flex flex-col flex-1">
              ...
              <div
                ...
                className="flex flex-col items-center py-4 mt-auto hover:bg-gray-100
                           cursor-pointer transition-colors border-t border-gray-100"
              >
...
function NavItem({ icon, label, href, onAfterClick }) {
  const content = (
    <span className="flex flex-col items-center py-4 hover:bg-gray-100
